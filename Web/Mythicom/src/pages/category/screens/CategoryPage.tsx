...
const CategoryPage = () => {
  ...
  async function FetchData() {
    ...
    await GetAllOrderedData('createdAt', 'products').then((products) => {
      ...
      if (param1 === 'result') {
        setFilteredProducts(productsByCategory.filter(((data:any) => data.name.includes(param2))))
      } else {
        if (param2 !== '1') {
          setFilteredProducts(productsByCategory.filter((data: any) => data.subCategory === param2))
        }
        else setFilteredProducts(productsByCategory)
      }
